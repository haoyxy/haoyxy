  ...
  return (
    <div className="w-full p-6 sm:p-8 bg-slate-900/50 rounded-xl shadow-2xl border border-cyan-500/10 flex flex-col items-center space-y-6">
      ...
      <textarea
        ...
        className="w-full h-96 p-4 bg-slate-900/70 border-2 border-slate-700 rounded-lg shadow-inner focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-colors duration-200 resize-y disabled:bg-slate-800 text-slate-300 placeholder-slate-500"
        aria-label="章节内容输入框"
      />
       <div className="w-full flex justify-between items-center flex-wrap gap-4">
        <p className={`text-sm ${wordCount < 1500 || wordCount > 6000 ? 'text-amber-400' : 'text-slate-500'}`}>
