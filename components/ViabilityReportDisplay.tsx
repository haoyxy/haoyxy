...
const Section: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode }> = ({ title, icon, children }) => (
  <div className="main-card rounded-xl p-6 transition-all hover:border-cyan-500/30">
    <div className="flex items-center mb-4">
      <div className="text-cyan-400 mr-4 p-2 bg-slate-900/50 rounded-lg">{icon}</div>
      <h3 className="text-2xl font-bold text-slate-100">{title}</h3>
    </div>
    <div className="pl-12 border-l-2 border-slate-700/50 ml-4">{children}</div>
  </div>
);
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-slate-300">
            <div className="md:col-span-1 main-card rounded-xl p-6 flex flex-col items-center justify-center text-center">
            ...
            <div className="md:col-span-2 main-card rounded-xl p-6">
        ...
        <Section title="市场匹配度分析" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>}>
            <div className="space-y-4 text-slate-300 py-2">
                <div>
                    <h4 className="font-semibold text-cyan-300 mb-2">推荐题材分类:</h4>
                ...
                <div className="pt-2">
                    <h4 className="font-semibold text-cyan-300 mb-2">目标读者画像:</h4>
                    <p className="text-sm leading-relaxed whitespace-pre-wrap">{marketFitAnalysis.targetAudience}</p>
                </div>
                <div className="pt-2">
                    <h4 className="font-semibold text-cyan-300 mb-2">市场潜力预测:</h4>
        ...
        <Section title="潜在毒点预警" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>}>
            <div className="space-y-4 py-2">
                {poisonPillWarning.warnings.length > 0 ? (
                 ...
                 <div className="pt-4 border-t border-slate-700/50">
                    <h4 className="font-semibold text-cyan-300 mb-2">风险总结:</h4>
         ...
         <Section title="综合评估与建议" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}>
             <div className="prose prose-custom max-w-none prose-lg whitespace-pre-wrap leading-relaxed text-slate-300 py-2">
                {overallAssessment}
            </div>
            <div className="flex space-x-3 mt-6">
                <CopyToClipboardButton textToCopy={JSON.stringify(report, null, 2)} displayText="复制报告 (JSON)" />
                <ExportButton 
                    contentToExport={JSON.stringify(report, null, 2)} 
                    defaultFilename="creative_viability_report.json"
                    buttonText="导出报告 (JSON)"
                />
            </div>
        </Section>
    </div>
